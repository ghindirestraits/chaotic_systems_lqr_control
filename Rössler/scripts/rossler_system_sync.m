%% Rössler Systems Synchronization
% This code aims to plot the synchronization of two chaotic elements
% belonging to the Rössler System's family of equations
...
for j = 1:tFinal
    ...
    % Response System
    xid(1) = (-1*(xi(2) + xi(3)) + u)*dt;
    xid(2) = (xi(1) + a*xi(2))*dt;
    xid(3) = (b + xi(3)*(xi(1) - c))*dt;
...
figure('Name', 'Rössler Attractor: 3D Phase Diagram', 'NumberTitle', ...
...
text(x_array(tFinal,1),x_array(tFinal,2),x_array(tFinal,3), ...
    'End of x')
text(xi_array(tFinal,1),xi_array(tFinal,2),xi_array(tFinal,3)-1.5, ...
    'End of \xi')
...
figure('Name', 'Rössler Attractor: States Time Response', 'NumberTitle', ...
